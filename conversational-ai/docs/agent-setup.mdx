...
<Steps>
  <Step title="Configure Evaluation Criteria">
    ...
    ```plaintext Evaluation Examples
    1. Name: order_completion
  ...
  <Step title="Set Up Data Collection">
    ...
    ```plaintext Data Collection Examples
    1. Identifier: order_type
       Description: Extract the type of order from the conversation. Should be one of: "delivery", "pickup", "inquiry_only"

    2. Identifier: ordered_items
       Description: List all pierogi varieties and quantities ordered in the format: "item: quantity". If no order was placed, return "none"

    3. Identifier: delivery_zone
       Description: Based on the delivery address, categorize the location as: "central_zakopane", "outer_zakopane", or "outside_delivery_zone"

    4. Identifier: interaction_type
       Description: Categorize the conversation as: "completed_order", "abandoned_order", "menu_inquiry", or "general_inquiry"
    ```
