...
AI is a highly advanced field of technology and can, at times, be unpredictable as the output is up to the discretion of the AI. We have tried to minimize the unpredictability as much as possible and keep adding features and improvements that make it more predictable and controllable. However, there are still many things you need to be mindful of, and this applies to all generative AI.
...
We've heard reports where users say that there is inconsistency between generations, so takes don't always fit together perfectly. This is something we are aware of and are working on resolving. However, it is a lot less prominent in the multilingual v2 model compared to the other models, and it seems to be less of an issue with Projects, as it is a bit more advanced than Speech Synthesis when it comes to long-form content as it was specifically made for that.

The current suggestion to resolve this would be to try cloning the voice again with different samples if you encounter a lot of variability. We recommend that you read the cloning guide in the documentation, which states that you should use around 1 to 2 minutes of audio. This audio should be very consistent if you want the output to be consistent; more audio than that can make the AI too variable which can cause inconsistency between generations.
...
On the rare occasions, you might encounter certain forms of glitches or become out of breath between paragraphs, which you might not experience with speech synthesis, as they operate on different principles. Generally, this issue is not extremely disruptive and is relatively rare, but we are actively working on resolving it. At the moment, there is no straightforward solution to completely avoid this problem. If you do happen to encounter an issue like this, we recommend regenerating the last paragraph. These issues tend to occur at the end of certain paragraphs rather than at the beginning. So, if you hear a problem between two paragraphs, it's usually the preceding paragraph that is the cause of the issue.

## Multilingual v1 (experimental)
