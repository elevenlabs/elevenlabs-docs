---
title: "How to use text to speech with streaming (ElevenLabs SDK)"
sidebarTitle: "How to use text to speech"
description: "How to convert text into speech, upload to S3, and share with a signed URL"
icon: "code"
---

In this tutorial, you'll learn how to convert [text to speech](/text-to-speech) with the ElevenLabs SDK. We’ll start by talking through how to generate speech and receive a file and then how to generate speech and stream the response back. Finally, we’ll show you how to upload the generated audio to an AWS S3 bucket, and share it through a signed URL. This signed URL will provide temporary access to the audio file, making it perfect for sharing with users by SMS or embedding into an application.

If you want to jump straight to the finished repo you can find it. 
* [here-Python](https://github.com/elevenlabs/elevenlabs-examples/tree/main/examples/text-to-speech/python). 
* [here-TypeScript](https://github.com/elevenlabs/elevenlabs-examples/tree/main/examples/text-to-speech/node).
...
`text_to_speech_file.py` for Python:
...
`text_to_speech_file.ts` for TypeScript:
...
`text_to_speech_stream.py` for Python:
...
`text_to_speech_stream.ts` for TypeScript:
...
<Frame caption="Enter a New S3 Bucket Name">
  <img src="/api-reference/images/aws_s3_enter_bucket_name.png" />
</Frame>

<Frame caption="S3 Bucket List">
  <img src="/api-reference/images/aws_s3_bucket_list.png" />
</Frame>
...
For Python, you can then call the `upload_audiostream_to_s3` function with the audio stream from the `text_to_speech_stream` function.

```python
s3_file_name = upload_audiostream_to_s3(audio_stream)
```

For TypeScript, you can then call the `uploadAudioStreamToS3` function with the audio stream from the `createAudioStreamFromText` function.
...
- `env.example`: Template for your environment variables.
- `src/index.ts`: Orchestrates the conversion and uploading process.
- `src/s3_uploader.ts`: Handles the logic for uploading files to S3 and generating signed URLs.
- `src/text_to_speech_file.ts`: Implements the conversion of text to speech, saving it as a file.
- `src/text_to_speech_stream.ts`: Implements streaming audio data directly.
