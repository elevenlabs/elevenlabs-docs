...
A key concept to understand when using websockets is buffering. The API only runs model generations when a certain amount of text above a threshold has been sent.

For example, if the threshold is set to 120 characters and you send 'Hello, how are you?' (19 characters), the audio won't be generated immediately. However, if you keep sending message, the API will generate audio once the total text have at least 120 characters.

In the case that you want force return the audio immediately, you can use `flush=true` to clear out the buffer and force generate any buffered text. One use case is that you've reached the end of document when sending text from it.
...
tsx text-to-speech-websocket.ts
```
</CodeGroup>

## Tips
- Send an empty string to close the websocket connection after sending the last text message.
- The API maintains a internal buffer so that it only runs model generations when a certain amount of text above a threshold has been sent. This is to optimize the quality of the generated audio. You can use `flush=true` to clear out the buffer and force generate any buffered text.
- To optimize the streaming latency, you can set the `optimize_streaming_latency` parameter in the URI to a value between 1 and 5. The recommended setting for low latency and real time applications is either 3 or 4.
