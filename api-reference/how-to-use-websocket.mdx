...
- The API maintains a internal buffer so that it only runs model generations when a certain amount of text above a threshold has been sent. You can use `flush=true` to clear out the buffer and force generate any buffered text.
