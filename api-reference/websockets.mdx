...
<ParamField body="try_trigger_generation" type="boolean" default={false} deprecated>
...
- For short texts with a character length smaller than the value set in `chunk_length_schedule`, set `flush=true` along with the text to force generate any buffered text.
- When developing a real-time conversational AI application, we advise using `flush=true` along with the text at the end of each long sentence or conversation turn to ensure timely audio generation.
