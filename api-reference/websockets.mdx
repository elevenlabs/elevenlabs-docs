...
<Note>
  Endpoint: `wss://api.elevenlabs.io/v1/text-to-speech/{voice_id}
...
<ParamField body='text' type='string' required>
  Should always end with a single space string `" "`. In the first message, the
...
<ParamField body='xi_api_key' type='string'>
  Provide the XI API Key in the first message if it's not in the header. See
  [Authentication](/api-reference/quick-start/authentication) for more details.
</ParamField>

<ParamField body='authorization' type='string'>
  Authorization bearer token. Should be provided only in the first message if
  not present in the header and the XI API Key is not provided.
</ParamField>

<Note>
  For best latency we recommend streaming word-by-word, this way we will start
  generating as soon as we reach the predefined number of un-generated
  characters.
</Note>
...
<ParamField body='char_start_times_ms' type='array'>
  ...
  normalized text as it corresponds to the audio. For instance, the character
  'H' starts at time 0 ms in the audio.
</ParamField>

{' '}

<ParamField body='chars_durations_ms' type='array'>
  A list providing the duration (in milliseconds) for each character's
  pronunciation in the audio. For instance, the character 'H' has a
  pronunciation duration of 3 ms.
</ParamField>
...
<ParamField path='voice_id' type='string'>
  Voice ID to be used, you can use [Get Voices](/api-reference/voices) to list
...
Below is a code sample showing how the websockets uri looks when using query params like `optimize_streaming_latency` and `output_format`
Let's say we want to use `2` as our `optimize_streaming_latency` and `mp3_44100_64` as our `output_format` then the uri would look like this:

```python
uri = f"wss://api.elevenlabs.io/v1/text-to-speech/{VOICE_ID}/stream-input?model_id=eleven_monolingual_v1&optimize_streaming_latency=2&output_format=mp3_44100_64"
```

Note: The `output_format` should be accessible in the free tier, in case you're using the free tier. For more info you can read the section on `output_format` [here](https://elevenlabs.io/docs/api-reference/streaming)

## Code sample using `optimize_streaming_latency` and `output_format`
...
async def text_to_speech():
  uri = f"wss://api.elevenlabs.io/v1/text-to-speech/{VOICE_ID}/stream-input?model_id=eleven_monolingual_v1&optimize_streaming_latency={OPTIMIZE_STREAMING_LATENCY}&output_format={OUTPUT_FORMAT}"
  ...
  async with websockets.connect(uri) as websocket:
    ...
    text = "This is a new text to speech test. "
...
You can also follow our integration guide on building real-time text-to-speech applications using websockets [here](/api-reference/websockets-text-to-speech-guide)

NOTE: To use different languages like **French**, **Japanese**, **Polish**, **Spanish** etc, you can use our `eleven_multilingual_v2` model. Avoid using the `v1` variation of the multilingual model since that is experimental. You can get more info about our various models using `/v1/models` endpoint.
Here is how the request body would look like while using the `eleven_multilingual_v2` model:
...
```json
{
  ...
  "language_id": "fr" // uses the french accent
}
```
