---
title: "Conversational AI WebSocket API"
...
<Note>
  ...
  ElevenLabs](https://elevenlabs.io/docs/conversational-ai-sdks).
</Note>
...
- Audio data should be in PCM 16-bit mono format and base64 encoded.
- Audio sample rate should be 16,000 Hz.

#### Pong Message

Respond to server `ping` messages to measure and manage latency.
...
**Note:** The `event_id` should match the `event_id` received in the `ping` message from the server.
...
| internal_tentative_agent_response | Provides tentative agent responses (for debugging)  |
...
- [ElevenLabs Conversational AI SDKs](https://elevenlabs.io/docs/conversational-ai-sdks/client-sdk)
