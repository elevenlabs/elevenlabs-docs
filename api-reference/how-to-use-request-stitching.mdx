---
title: "Combining multiple generations"
...
It would be much better to give the model information on what was already generated and what will be generated in the future, this is exactly what Request Stitching does.
...
data = {
    "text": "Unknown to most, this enchanted place was stumbled upon by an elderly librarian named Eleanor, who had lived in the city all her life and thought she knew every corner like the back of her hand.",
    "model_id": "eleven_multilingual_v2",
    "previous_text": "In the heart of an ancient city, veiled by the dust of bustling streets and the shadows of towering modern skyscrapers, there lies a secret garden.",
    "next_text": "One crisp autumn morning, while Eleanor was wandering through an old neighborhood, a strange, sweet scent caught her attention."
}

response = requests.post(tts_url, headers=headers, json=data, stream=True)
```

This works well when there has been no previous or next chunks generated yet. If there have been however, it works much better to provide the actual generation to the model instead of just the text.
This is done by using the previous_request_ids and next_request_ids parameters. Every text-to-speech and speech-to-speech request has an associated request-id which is obtained by reading from the response header.

Below is an example on how to read the request_id from the response header and then use it to condition another request on the previous generation.
