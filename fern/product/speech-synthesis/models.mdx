...
<<<<<<< HEAD:product/speech-synthesis/models.mdx
...
>>>>>>> 6203563 (add v2 docs configuration):fern/product/speech-synthesis/models.mdx
...
Our most advanced speech synthesis model, Multilingual v2, offers high stability, diverse language support, and exceptional accuracy in 29 languages. While slower than the Flash models, it delivers more lifelike speech, making it ideal for content creation such as voiceovers, audiobooks, and post-production.
...
Our very first model, English v1, laid the groundwork for future advancements. While still functional, it is now outclassed by Multilingual v2 (for content creation) and v2.5 Flash (for low-latency applications). We recommend using our newer models for better quality and speed.

**Eleven Multilingual v1**

Multilingual v1 was our first attempt at generating speech in multiple languages, but it is now considered experimental and has been surpassed by Multilingual v2 and v2.5 Flash. We recommend avoiding this model for production use due to its limitations and lower accuracy.
...
A low-latency, English-only model optimized for conversational applications. v2 Flash is similar in performance to v2.5 Flash but focused exclusively on English, making it ideal for English-only use cases where speed is critical.
- Great quality
- High accuracy with Professional Voice Clones
- Slightly less stable
- Optimized for low latency
