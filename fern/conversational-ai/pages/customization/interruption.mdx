---
title: Interruptions
subtitle: Configure how your assistant handles being interrupted during speech.
---

## Overview

Interruption handling determines whether users can interrupt your assistant while it's speaking. This behavior can be enabled or disabled based on your use case.

## Configuration

Interruption settings can be configured in the **Advanced** tab under **Client Events**. When interruptions are enabled, users can interrupt the assistant mid-speech to ask questions or provide new information. When disabled, the assistant will complete its entire response regardless of user input.

#### Interruptions Enabled

<Frame background="subtle">
  ![Interruption allowed](/assets/images/conversational-ai-interruptions.png)
</Frame>

#### Interruptions Disabled

<Frame background="subtle">
  ![Interruption ignored](/assets/images/conversational-ai-no-interruption.png)
</Frame>

<Note>
  Disable interruptions when the complete delivery of information is crucial, such as legal
  disclaimers or safety instructions.
</Note>

## Best practices

- Enable interruptions for natural conversational flows where back-and-forth dialogue is expected
- Disable interruptions when message completion is critical (e.g., terms and conditions, safety information)
- Consider your use case context - customer service may benefit from interruptions while information delivery may not
