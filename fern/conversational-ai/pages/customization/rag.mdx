...
async function enableRAG(documentId, agentId, apiKey) {
  try {
    ...
    let response = await elevenlabs.conversationalAi.knowledgeBase.document.computeRagIndex(documentId, {
    ...
    while (response.status !== 'SUCCEEDED' && response.status !== 'FAILED') {
      ...
      response = await elevenlabs.conversationalAi.knowledgeBase.document.computeRagIndex(documentId, {
        model: 'e5_mistral_7b_instruct',
      });
    }
