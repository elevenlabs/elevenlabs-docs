...
**Retrieval-Augmented Generation (RAG)** enables your agent to access and utilize large knowledge bases during conversations. Instead of loading entire documents into the context window, RAG retrieves only the most relevant information for each user query, allowing your agent to:
...
<Warning>RAG adds on slight latency to the response time of your agent, around 500ms.</Warning>
...
2. **Embedding Generation**: The processed query is converted into a vector embedding
...
<CodeBlocks>
    ...
    status = response.json()["status"] # CREATED, PROCESSING, SUCCEEDED, or FAILED
...
- Some documents may be too large to be included in "Prompt" mode
