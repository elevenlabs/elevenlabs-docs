...
<Warning title="Limits Not Yet Active">
  The following usage limits for RAG are planned but not yet fully implemented. We will provide
  further updates when these limits become active and are reaching out to affected users.
</Warning>

To ensure fair resource allocation, ElevenLabs plans to introduce limits on the total size of documents that can be indexed for RAG per workspace, based on subscription tier.
...
| Scale                   | 500MB                     |                                             |
| Business                | 1GB                       |                                             |
| Higher Enterprise Tiers | Custom                    | Higher limits available based on agreement. |
...
These limits are designed to allow most users ample capacity for their RAG needs while encouraging efficient knowledge base management.
