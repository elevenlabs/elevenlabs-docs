...
<AccordionGroup>
  ...
  <Accordion title="internal_tentative_agent_response">
    - Contains partial agent response before TTS generation
    - Provides streaming text before speech starts
    - Useful for displaying text alongside audio

    ```javascript
    // Example internal tentative agent response event structure
    {
    ...
    ```javascript
    // Example tentative response handler (via onDebug in SDK)
    websocket.on('internal_tentative_agent_response', (event) => {
      ...
      displayTentativeMessage(tentative_agent_response);
    });
    ```
