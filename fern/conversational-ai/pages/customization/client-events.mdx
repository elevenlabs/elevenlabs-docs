...
<AccordionGroup>
  ...
  <Accordion title="internal_tentative_agent_response">
    - Contains preliminary agent response before final confirmation
    - Used for preview or streaming response display
    - May be followed by the final agent_response event

    ```javascript
    // Example tentative agent response event structure
    {
    ...
    ```javascript
    // Example tentative response handler
    websocket.on('internal_tentative_agent_response', (event) => {
      const { tentative_agent_response_internal_event } = event;
      const { tentative_agent_response } = tentative_agent_response_internal_event;
      displayTentativeResponse(tentative_agent_response);
    });
    ```
