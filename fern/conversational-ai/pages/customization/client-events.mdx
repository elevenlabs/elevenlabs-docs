...
<AccordionGroup>
  ...
  <Accordion title="contextual_update">
    - Allows sending non-interrupting contextual information to update conversation state
    - Updates are incorporated as tool calls in the conversation history
    - Useful for adding background information without disrupting the flow

    ```javascript
    // Example contextual update event structure
    {
    ...
    ```javascript
    // Example contextual update handler
    websocket.send(JSON.stringify({
      type: 'contextual_update',
      text: 'Customer status: Premium tier'
    }));
    ```
