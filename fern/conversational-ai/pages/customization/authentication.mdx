...
<EndpointRequestSnippet endpoint="POST /v1/convai/conversation/get_signed_url" />
...
On the client side, use the signed URL returned from your server to connect tot he websocket.
...
<AccordionGroup>
  ...
  <Accordion title="What other security measures should I implement?">
    Beyond signed URLs and allowlists, consider implementing: - User authentication before
    requesting signed URLs - Rate limiting on API requests - Usage monitoring for suspicious
    patterns - Proper error handling for auth failures
  </Accordion>
</AccordionGroup>
