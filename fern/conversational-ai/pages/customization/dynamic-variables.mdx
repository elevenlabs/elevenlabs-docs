...
- **Personalizing the widget** by showing dynamic variables in the widget
...
<Steps>
  ...
  <Step title="Personalize the Conversational AI widget">
    You can also use dynamic variables in the Conversational AI widget parameters by using the same `{{variable_name}}` syntax:

    <Frame background="subtle">
      ![Dynamic variables in widget](/assets/images/conversational-ai/dynamic-vars-widget.png)
    </Frame>

    An ideal use case for this is customising the widget to with the user's name, for example.
