...
**Dynamic conversations** enable your assistant to adapt its behavior for each user interaction. You can pass custom data and settings at the start of each conversation, allowing the assistant to personalize its responses and knowledge with real-time context.

## Overview

Dynamic conversations allow you to modify your AI agent's behavior in real-time without creating multiple agents. This enables you to personalize responses with user-specific data.

Here are a few examples where dynamic conversations can be useful:
...
See the [WebSocket reference](/docs/conversational-ai/docs/agent-websocket-api) for all available messages and options.

<Info>
  Dynamic conversations are particularly useful for applications requiring personalized interactions
  or handling sensitive user data that shouldn't be stored in the agent's base configuration.
</Info>
...
<Steps>
  ...
  <Step title="Override the conversation">
    ...
    <Tip>
      Ensure you have the latest [SDK](https://elevenlabs.io/docs) installed.
    </Tip>
