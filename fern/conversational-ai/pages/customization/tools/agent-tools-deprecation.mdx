...
<Check>
  **July 7, 2025** - Last day for full backwards compatibility. You can continue using
  `prompt.tools` until this date.
</Check>

<Note>
  **July 8, 2025** - GET endpoints will stop returning the `tools` field. Only `prompt.tool_ids`
...
| ----------------- | ------------------------ | -------------------------------------------------------------------------------- |
| **July 7, 2025**  | ✅ Full compatibility    | You may keep sending `prompt.tools`. GET responses include the `tools` field.    |
| **July 8, 2025**  | ⚠️ Partial compatibility | GET endpoints stop returning the `tools` field. Only `prompt.tool_ids` included. |
...
<AccordionGroup>
  <Accordion title="Will my existing integrations break?">
    ...
    starting July 8, GET and PATCH responses will no longer include full tool objects. After July
