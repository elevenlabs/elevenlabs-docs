...
<Check>
  **June 30, 2025** - Last day for full backwards compatibility. You can continue using
  `prompt.tools` until this date.
</Check>

<Note>
  **July 1, 2025** - GET endpoints will stop returning the `tools` field. Only `prompt.tool_ids`
  will be included in responses.
</Note>

<Warning>
  **July 16, 2025** - Legacy `prompt.tools` field will be permanently removed. All requests
...
| ----------------- | ------------------------ | -------------------------------------------------------------------------------- |
| **June 30, 2025** | ✅ Full compatibility    | You may keep sending `prompt.tools`. GET responses include the `tools` field.    |
| **July 1, 2025**  | ⚠️ Partial compatibility | GET endpoints stop returning the `tools` field. Only `prompt.tool_ids` included. |
| **July 16, 2025** | ❌ No compatibility      | POST and PATCH endpoints **reject** any request containing `prompt.tools`.       |
...
<AccordionGroup>
  <Accordion title="Will my existing integrations break?">
    No. Until 30 June, the API will silently migrate any `prompt.tools` array you send and return
    full tool objects in GET responses. After 1 July the GET responses will no longer include those
    objects, and after 16 July POST/PATCH requests containing `prompt.tools` will be rejected.
  </Accordion>
