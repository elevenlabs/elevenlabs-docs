...
<Info>To perform operations server-side, use [tools](/docs/conversational-ai/tools) instead.</Info>
...
When you want your agent to receive data back from a client tool, ensure that you tick the **Wait for response** option in the agent dashboard. Once enabled, the client tool function returns a value that the agent uses to enhance the conversation context.
...
In both examples, when the agent calls **getCustomerDetails**, the function will execute on the client and the agent will receive the returned data, which is then used as part of the conversation context.
