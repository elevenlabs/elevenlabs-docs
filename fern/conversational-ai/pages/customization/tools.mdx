...
| timeZone   | String    | Yes      | The caller's timezone. Should be in the format of 'Continent/City' like 'Europe/London' or 'America/New_York'. |
    ...
    <Step title="Enhancements">
      ...
      2. **Dynamic conversation**: Use the [dynamic conversation](/docs/conversational-ai/customization/dynamic-conversation) functionality to inject the current date and time into the system prompt at the start of each conversation.
