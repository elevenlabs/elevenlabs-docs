...
When using authentication with a signed URL, include the same override:
...
elevenlabs,
agent_id,
conversation_overrides={"textOnly": True}, # This callback is essential for chat mode
...
```python
# Send a text message to the agent
conversation.send_text_input("Hello, how can you help me today?")
```
</Tab>
</Tabs>
...
For proper message flow:

1. Wait for agent responses before sending the next user message
2. Handle connection status changes appropriately
3. Implement proper error handling for failed messages

## Next Steps

- Learn about [customizing agent behavior](/docs/conversational-ai/customization/llm)
- Explore [client events](/docs/conversational-ai/customization/client-events) for advanced interactions
