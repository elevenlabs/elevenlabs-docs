...
1. **For inbound support**: Ensure the number is purchased and appears in your Twilio Console under "Phone Numbers" → "Manage" → "Active numbers"
2. **For outbound support**: Verify the number in Twilio Console under "Phone Numbers" → "Manage" → "Verified Caller IDs"
3. Check that your Twilio SID and Auth Token are correct

### Outbound Call Button Disabled

If the outbound call button is disabled with a tooltip about outbound not being supported:

- This number was not found in your Twilio verified caller IDs
- Add the number to your verified caller IDs in Twilio Console
- Contact support if you believe this is an error

### Cannot Assign Agent (Dropdown Disabled)

If the agent assignment dropdown is disabled:

- This number only supports outbound calls (verified caller ID)
- To enable inbound calls, purchase this number through Twilio instead of just verifying it
