...
<Warning>
  This guide demonstrates a custom Twilio integration, which should be used for outbound calls only. For inbound calls and production use cases, we strongly recommend using:

    1. [Twilio's native integration](/docs/conversational-ai/guides/twilio/dashboard) which provides optimized call handling and reliability
    2. [Twilio's personalization features](/docs/conversational-ai/customization/dynamic-variables/twilio-inbound-integration) to customize conversations based on caller data
