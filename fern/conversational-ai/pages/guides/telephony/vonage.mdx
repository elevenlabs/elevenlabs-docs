...
<Frame background="subtle">
  <img
    src="/assets/images/11l-agent-ws-overview-diagram.png"
    alt="Architecture diagram showing Vonage connecting via WebSocket to the Connector, which connects via WebSocket to ElevenLabs AI."
  />
</Frame>
...
- **API Key**: From "My Account" -> "API Keys" on the [ElevenLabs dashboard](https://elevenlabs.io/app).
- **Agent ID**: From "Conversational AI" -> "Agents" -> (...) -> "Copy Agent ID".
...
Use ngrok to create a public URL for the connector (default port 6000).
...
- **Update Existing App**: Modify your NCCO to include a `connect` action targeting the connector's WebSocket URI (`wss://...`) with `content-type: audio/l16;rate=16000`. Pass necessary query parameters like `peer_uuid` and `webhook_url`.
