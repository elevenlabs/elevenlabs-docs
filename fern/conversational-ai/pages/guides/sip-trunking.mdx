...
2. **Authentication**: Connection security for the signaling is maintained through either digest authentication (username/password) or Access Control List (ACL) authentication.
3. **Media Stream**: Once the call is established, the actual audio data (RTP stream) is transmitted over UDP for low latency.
...
<Steps>
  ...
  <Step title="Import SIP Trunk">
    ...
    <Note>
      The `transport=tcp` parameter specifies that the initial SIP signaling connection uses TCP. However, the actual audio data (RTP stream) is transmitted over UDP for low latency performance, which is standard for real-time communication.
    </Note>
