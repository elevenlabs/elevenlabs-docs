...
<Steps>
  ...
  <Step title="Configure outbound settings">
    ...
    - **Transport Type**: Select the transport protocol for SIP signaling:
      - **Auto**: Automatically selects the best available transport
...
<AccordionGroup>
  ...
  <Accordion title="Authentication Failures">
    ...
    5. Test with simplified configurations (no custom headers) to isolate authentication issues
...
<AccordionGroup>
  ...
  <Accordion title="What's the difference between transport types?">
    - **Auto**: Automatically selects the best available transport protocol  
    - **UDP**: Fastest but unencrypted signaling (good for internal networks)
    - **TCP**: Reliable but unencrypted signaling
    - **TLS**: Encrypted and reliable signaling (recommended for production)
    
    For security-critical applications, always use TLS transport.
  </Accordion>
