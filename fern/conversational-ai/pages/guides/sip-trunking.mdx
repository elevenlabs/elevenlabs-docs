...
SIP trunking supports two transfer methods: Conference Transfer and SIP REFER Transfer.

<Warning>
  **SIP REFER + TLS Limitation**: SIP REFER transfer does not work with TLS transport due to a
  Livekit bug. Use either TCP with SIP REFER or Conference transfer with TLS.
</Warning>

### Transfer Method Options

- **Conference Transfer**: Works with all transport protocols (TCP, TLS) and media encryption
  settings
- **SIP REFER Transfer**: Only works with TCP transport. For Twilio, requires TCP and disabled
  media encryption

### Twilio-Specific Requirements

When using SIP REFER transfers with Twilio:

- **Transport**: Must use TCP (not TLS)
- **Media Encryption**: Must be disabled
...
<Accordion title="Transfer Issues">
  If call transfers are not working properly:

- **SIP REFER + TLS**: Does not work due to Livekit bug. Use TCP with SIP REFER or Conference transfer with TLS
- **SIP REFER + Twilio**: Requires TCP transport and disabled media encryption
- **Test basic connectivity**: Verify direct outbound calls work before testing transfers
  </Accordion>
...
<Accordion title="What's the difference between transport types?">
  - **TCP**: Reliable but unencrypted signaling, compatible with all transfer methods
  - **TLS**: Encrypted and reliable signaling (recommended), but SIP REFER transfers require TCP
  
  <Note>
    For security-critical applications with transfers, use TLS transport with Conference transfer method.
  </Note>
</Accordion>
...
<Accordion title="Why are my call transfers failing?">
  **SIP REFER + TLS**: Does not work due to Livekit bug. Use TCP with SIP REFER or Conference
  transfer with TLS. **SIP REFER + Twilio**: Requires TCP transport and disabled media encryption.
</Accordion>
