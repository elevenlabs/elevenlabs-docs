...
With our Salesforce integration, your conversational AI agents can access customer data, manage leads, and create opportunities directly within Salesforce. This integration streamlines CRM processes by automatically retrieving customer information, checking for existing records, and securely creating new records like leads and opportunities. Benefits include faster customer qualification, reduced manual data entry, and enhanced customer experience through personalized interactions.
...
<Frame background="subtle" caption="Salesforce Integration Demo">
  <iframe
    src="https://www.loom.com/embed/2c7664ecddff40cda9b03ebe3f61d854?sid=9e7520ba-8ad6-4055-83bb-d7a3feb8b82c"
    frameBorder="0"
...
- [ ] Configure OAuth settings with Start URL and Callback URL
...
- [ ] Configure tool authorization (Workplace Auth Connections or manual secrets)
- [ ] Test your integration with manual API calls
...
<Tip>
  ...
  lead qualification. Tool authorization can be managed using either manual secrets or Workplace
  Auth Connections (recommended for automatic token refresh). The tools are configured to return
  human-readable names and descriptions rather than technical IDs to improve conversation quality.
</Tip>
...
The integration with Salesforce employs three primary webhook tools to manage customer relationships. You can configure authorization for these tools using either manual secrets or Workplace Auth Connections.

### Authorization Options

<Tabs>
  <Tab title="Workplace Auth Connections">
    **Recommended**: Configure automatic token management through the ElevenLabs dashboard.
    ...
    <Steps>
      ...
      <Step title="Use in Tool Configurations">
        Once the connection is successful, save it and reference it in your webhook tool configurations in the **Authentication** section instead of using manual secrets.
      </Step>
    </Steps>
  ...
  <Tab title="Manual OAuth Token">
    Generate OAuth tokens manually and manage them as secrets. This requires manual token refresh when tokens expire.
    
    <Steps>
      <Step title="Generate the OAuth Token">
      ...
      <Step title="Create Authentication Secret">
        ...
        - Example: `Bearer 00D5g0000XXXXXX12341QIQEUqKKOYTsoK01jQx79...`
      </Step>

      <Step title="Configure Tool Authorization">
        In your webhook tool configurations, set:
        - **Authorization**: *(Secret: `salesforce_auth`)*
      </Step>
    </Steps>

    <Note>
      Access tokens expire and will need to be refreshed periodically. Consider implementing token
      refresh logic or using a longer-lived approach for production use.
    </Note>
...
Use the tabs below to review each tool's configuration.
...
<Warning>
  Ensure that you configure tool authorization for your Salesforce webhook tools using either: -
  **Workplace Auth Connections**: Configure the connection in your ElevenLabs dashboard and
  reference it in your webhook tool configurations' **Authentication** section, OR - **Manual OAuth
  Token**: Replace `YOUR_SALESFORCE_SECRET` with your actual secret ID in the tool configurations.
</Warning>

## Common SOQL Queries

Here are some commonly used SOQL queries for the `salesforce_search_records` tool. All queries prioritize human-readable information over technical IDs:
...
You can test individual components using curl commands or tools like Postman:

1. **Test Search Tool** (notice the query includes Name and human-readable fields):

   ```bash
   curl -X GET "https://your-domain.my.salesforce.com/services/data/v58.0/query/?q=SELECT%20Id%2C%20Name%2C%20Email%2C%20Phone%2C%20Title%2C%20Account.Name%20FROM%20Contact%20LIMIT%205" \
...
2. **Test Create Tool**:
   ...
   ```bash
   curl -X POST "https://your-domain.my.salesforce.com/services/data/v58.0/sobjects/Lead/" \
   -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
   -H "Content-Type: application/json" \
   -d '{
     "FirstName": "Test",
...
3. **Test Get Record Tool**:
   ```bash
   curl -X GET "https://your-domain.my.salesforce.com/services/data/v58.0/sobjects/Lead/YOUR_RECORD_ID" \
   -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
   -H "Content-Type: application/json"
   ```
...
4. **Test SOQL Queries**: Ensure complex queries work correctly

### What to Validate

A successful integration should demonstrate:

- ✅ **Search Records**: Can find existing contacts/leads using SOQL with human-readable names
- ✅ **Get Record**: Can retrieve detailed record information including names and descriptions
- ✅ **Create Record**: Can create new leads/contacts with proper data
- ✅ **Human-Readable Output**: Returns opportunity names, account names, and other descriptive fields rather than just IDs
...
<AccordionGroup>
  <Accordion title="Authentication Issues">
    ...
    **Solutions**:
    ...
    - Check that the External Client App is properly configured
    - Ensure the OAuth scope includes necessary permissions
    - Verify your access token hasn't expired
    - Confirm your domain format is correct (e.g., `https://yourcompany.my.salesforce.com`)
  </Accordion>

{' '}

<Accordion title="Valid Execution User Error">
  **Problem**: "Enter a valid execution user for the OAuth client credentials flow" **Solutions**: -
  Ensure the selected user is **active** (not deactivated) - Verify the user has **API Enabled**
  permission - Check that the user has a proper **Salesforce license** (not just Community) - Try
  selecting a different System Administrator user - Create a new dedicated service account user with
  proper permissions
</Accordion>

{' '}

<Accordion title="Domain Resolution Issues">
  **Problem**: DNS resolution errors or "nodename nor servname provided" **Solutions**: - Verify
  your domain format is correct - Try different domain formats: `.my.salesforce.com`,
  `.lightning.force.com`, `.develop.my.salesforce.com` - Check your browser's address bar when
  logged into Salesforce - Remove any trailing slashes from the domain - Ensure you're using HTTPS,
  not HTTP
</Accordion>
...
<Accordion title="SOQL Query Errors">
  **Problem**: Getting SOQL syntax errors **Solutions**: - Ensure all field names are spelled
  correctly - Check that you have permission to access the fields - Verify the object name is
  correct - Use proper SOQL syntax (no SELECT * allowed)
</Accordion>

{' '}

<Accordion title="Record Creation Failures">
  **Problem**: Unable to create records **Solutions**: - Verify required fields are included - Check
  field-level security permissions - Ensure data types match field requirements - Validate that
  picklist values are correct
</Accordion>
