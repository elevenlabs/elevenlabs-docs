...
- [ ] Configure agent secrets with the Bearer token
...
2. Go to **Setup** → **External Client App Manager** (or search "External Client App Manager" in Quick Find)
...
<AccordionGroup>
  ...
  <Accordion title="Valid Execution User Error">
    **Problem**: "Enter a valid execution user for the OAuth client credentials flow"
    
    **Solutions**:
    - Ensure the selected user is **active** (not deactivated)
    - Verify the user has **API Enabled** permission
    - Check that the user has a proper **Salesforce license** (not just Community)
    - Try selecting a different System Administrator user
    - Create a new dedicated service account user with proper permissions
  </Accordion>

  <Accordion title="Domain Resolution Issues">
    **Problem**: DNS resolution errors or "nodename nor servname provided"
    
    **Solutions**:
    - Verify your domain format is correct
    - Try different domain formats: `.my.salesforce.com`, `.lightning.force.com`, `.develop.my.salesforce.com`
    - Check your browser's address bar when logged into Salesforce
