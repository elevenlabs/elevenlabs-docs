...
<Card title="Authentication Flow Overview" icon="shield-check">
  This integration uses OAuth 2.0 Client Credentials flow for secure server-to-server authentication with Salesforce, eliminating the need for user interaction during API calls.
</Card>
...
<Steps>
  ...
  <AccordionGroup>
    <Accordion title="Required OAuth Scopes">
      - **Full access (full)** - Complete access to your data and applications
      - **Perform requests on your behalf at any time (refresh_token, offline_access)** - Enables token refresh
      - **Manage user data via api** - Allows CRUD operations on user data
    </Accordion>
  </AccordionGroup>
...
<Steps>
  ...
  <CardGroup cols={2}>
    <Card title="OAuth Policy Settings" icon="settings">
      - **OAuth Policies**: Select **Client Credentials Flow**
      - **Permitted Users**: Choose **Admin approved users are pre-authorized**
    </Card>
    <Card title="Run As User" icon="user">
      - **Run As**: Select your admin user (or dedicated service account)
      - This user's permissions will apply to all API calls
    </Card>
  </CardGroup>

  ### Configure Run As User Permissions
  <Info>
    The "Run As" user determines the permissions for all API calls. Choose a user with appropriate access.
  </Info>
...
<Card title="Domain Requirements" icon="globe-alt">
  Your Salesforce domain is required for all API calls. Choose the method below that works best for you.
</Card>
...
<Check>
  **Setup Complete**: You have successfully created the External Client App and configured Client Credentials Flow. The Consumer Key and Consumer Secret will be used for token generation in the tool authorization step.
</Check>
...
<Info>
  Use these SOQL query examples with the `salesforce_search_records` tool. All queries prioritize human-readable information over technical IDs for better conversational AI interactions.
</Info>
...
<Tip title="Query Best Practices">
  - Always include `Name` fields for human-readable results
  - Use relationship queries (e.g., `Account.Name`) to get contextual information
  - Include relevant fields that help personalize conversations
...
<Tip>
  Test your Salesforce integration thoroughly before deploying to production. Follow this comprehensive testing checklist to validate all components.
</Tip>
...
<Check>
  **Integration is ready for production when:**
  ...
  - Agent handles customer interactions naturally
...
<Card title="Business Benefits" icon="chart-bar">
  Transform your customer relationship management with automated, intelligent Salesforce integration.
</Card>
...
<Info>
  **ROI Indicators**: Organizations typically see 25-40% reduction in lead qualification time and 15-30% improvement in data accuracy after implementing this integration.
</Info>

## Security Considerations

<Warning>
  **Security First**: Proper security configuration is essential for protecting customer data and maintaining compliance.
</Warning>
...
<Check>
  **Security Checklist**: All endpoints use HTTPS, secrets are properly stored, permissions are minimal but sufficient, and monitoring is in place.
</Check>
