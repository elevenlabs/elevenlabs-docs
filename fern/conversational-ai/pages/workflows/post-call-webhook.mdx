...
<Note>
  Prior to August 15th, 2024, the webhook data structure differs from the [Conversation
  API](/docs/conversational-ai/api-reference/conversations/get-conversations) GET endpoint response.
  After the migration, webhooks will use the same data model as the GET endpoint for enhanced consistency.
</Note>

## Migration Notice: Enhanced Webhook Format

<Warning>
  **Important:** Starting August 15th, 2024, post-call webhooks will be migrated to include additional audio-related fields for enhanced compatibility and consistency.
</Warning>

### What's Changing

From August 15th, 2024, all post-call webhooks will be updated to match the same format as the [GET Conversation response](/docs/api-reference/conversations/get). The webhook `data` object will include three additional boolean fields:
...
2. **Test your webhook endpoints** with the new field structure before August 15th
...
- **Enhanced audio information**: Know exactly what audio components are available for each conversation

### Example of New Format

After migration, your webhook `data` object will include:
...
<Note>
  These new fields will be automatically included in all webhook payloads. No action is required on your part other than ensuring your webhook handlers can process boolean values for these three fields.
</Note>
...
<Note>
  As of August 15th, 2024, transcription webhooks will include the `has_audio`, `has_user_audio`, and `has_response_audio`
  fields to match the [GET Conversation
  response](/docs/api-reference/conversations/get) format exactly. Prior to this date, these fields are not included in webhook payloads.
</Note>
