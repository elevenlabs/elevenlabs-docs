...
<Note>
  Prior to August 15th, 2025, the webhook data structure differs from the [Conversation
...
<Warning>
  **Important:** Starting August 15th, 2025, post-call webhooks will be migrated to include additional audio-related fields for enhanced compatibility and consistency.
</Warning>

### What's Changing

From August 15th, 2025, all post-call webhooks will be updated to match the same format as the [GET Conversation response](/docs/api-reference/conversations/get). The webhook `data` object will include three additional boolean fields:
...
<Note>
  As of August 15th, 2025, transcription webhooks will include the `has_audio`, `has_user_audio`, and `has_response_audio`
