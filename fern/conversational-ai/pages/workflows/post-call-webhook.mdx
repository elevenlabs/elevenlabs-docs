...
<Note>
  The webhook data structure differs from the [Conversation API](/docs/conversational-ai/api-reference/conversations/get-conversations) GET endpoint response. Webhooks use a specialized data model optimized for event delivery.
</Note>
...
<Note>
  Transcription webhooks do NOT include the `has_audio`, `has_user_audio`, or `has_response_audio` fields that are available in the GET API response.
</Note>
