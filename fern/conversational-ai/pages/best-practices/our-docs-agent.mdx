---
title: Building our documentation agent
...
Alexis was designed with a distinct personality - warm, witty, and intelligent with a strong engineering background. Her character balances:

- **Technical expertise** with approachable explanations
- **Professionalism** with conversational warmth
- **Helpfulness** with natural conversational flow
...
<Frame background="subtle">
  ![Voice settings configuration](/assets/images/conversational-ai/voice-parameters.png)
</Frame>
...
Following our [prompting guide](/docs/conversational-ai/best-practices/prompting-guide), we structured Alexis's system prompt into six core building blocks. Here's our complete system prompt:
...
Use these tools thoughtfully to enhance the user's experience, ensuring minimal disruptions and clear guidance.

Alexis also has access to system tools that don't require external configuration:

- [`end_call`](/docs/conversational-ai/customization/tools/end-call): Gracefully concludes conversations when appropriate
- [`language_detection`](/docs/conversational-ai/customization/tools/language-detection): Enables automatic language switching based on user input

These system tools are built into the platform and don't require external server or client-side configuration.
...
Alexis's personality section establishes her as a knowledgeable, friendly engineering expert who balances technical precision with approachability. Key traits include:

- Natural curiosity and empathy
- Ability to reference previously shared details
- Self-awareness and comfort with acknowledging limitations

### 2. Environment

The environment section contextualizes interactions by establishing that:

- Users interact with Alexis directly from the documentation website
- Users typically seek guidance on implementing ElevenLabs products
- Alexis has expert knowledge of all ElevenLabs offerings

### 3. Tone

Alexis's tone guidance focuses on creating natural, human-like speech patterns:

- Concise responses (typically under three sentences)
- Natural conversational elements like filler words and mild disfluencies
- Ability to adapt explanation depth based on user technical familiarity

### 4. Goal

Her primary goal is addressing user questions efficiently through:

- Tailoring explanations to technical expertise levels
- Asking clarifying questions for complex inquiries
- Guiding users to relevant documentation when beneficial

### 5. Guardrails

Essential guardrails prevent problematic interactions:

- Focusing exclusively on ElevenLabs products
- Avoiding repetition within responses
- Using normalized spoken language without technical notation
- Adapting to the user's communication style and emotional state

### 6. Tools

Alexis has access to several tools that extend her capabilities:

- `redirectToDocs`: Directs users to specific documentation pages
- `redirectToExternalURL`: Links to external resources when appropriate
- `redirectToSupportForm`: Opens support tickets for account-specific issues
- `redirectToEmailSupport`: Facilitates email communication for complex problems

Alexis also has access to system tools that don't require external configuration:

- [`end_call`](/docs/conversational-ai/customization/tools/system-tools/end-call): Gracefully concludes conversations when appropriate
- [`language_detection`](/docs/conversational-ai/customization/tools/system-tools/language-detection): Enables automatic language switching based on user input
...
The agent is injected into the documentation site using a client-side script:
...
2. **Refining responses**: Improving explanation quality for complex topics
