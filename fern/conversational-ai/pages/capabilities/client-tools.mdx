...
<AccordionGroup>
  <Accordion title="Client tool not triggering">
    - Ensure that the tool name in your code matches exactly with the tool name in the agent
    configuration. - Check that the agent is correctly outputting the tool command in the
    conversation. - Verify there are no typos in the parameter names.
  </Accordion>
  <Accordion title="JavaScript errors in the browser">
    - Open the browser console to check for any errors. - Ensure that your code has necessary error
    handling for undefined or unexpected parameters.
  </Accordion>
</AccordionGroup>
