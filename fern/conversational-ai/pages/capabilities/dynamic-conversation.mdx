...
subtitle: Learn how to customize your AI agent for each conversation.
---
...
<Steps>
  <Step title="Enable overrides">
    Go to the `Security` tab in your agent settings and enable overrides for the first message and
    system prompt.
    <Frame>
...
<AccordionGroup>
  <Accordion title="Configuration Issues">
    If the configuration override isn't working: - Verify the configuration structure matches the
    expected format - Check that all required fields are present - Ensure the config object is
    properly passed to the Conversation constructor
  </Accordion>
</AccordionGroup>
