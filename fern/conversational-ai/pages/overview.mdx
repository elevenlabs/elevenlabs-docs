...
As a general heuristic, a concurrency limit of 5 can typically support up to around 100 simultaneous calls. However, actual performance depends on several factors, including which AI voices are used and the speaking ratio between the AI agent and the human participant. For instance, if the AI agent is expected to speak significantly more than the human, the system will support fewer simultaneous calls than the heuristic suggests.
