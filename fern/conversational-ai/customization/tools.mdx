...
<Note type="warning">
**When using tools, we recomment to avoid the default LLM (Gemini 1.5 Flash)**, and instead pick another LLM like GPT-4o mini or Claude 3.5 Sonnet, which are more likely to lead to successful function calls.
</Note>