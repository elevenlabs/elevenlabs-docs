...
Within the Cal.com documentation, we see that both our availability requires only the authorization header: 

    ```bash
    Authorization: Bearer <your-bearer-token>
    ```

And the booking endpoint requires three headers:
...
Our AI receptionist requires this only for the Get_Available_Slots tool.
...
And configure the path parameters as follows:
...
It's important to note that the choice of LLM matters to the success of function calls. Some LLMs can struggle with extracting the relevant parameters from the conversation. When using tools, we recomment to avoid the default LLM (Gemini 1.5 Flash), and instead pick another LLM like GPT-4o mini or Claude 3.5 Sonnet, which are more likely to lead to successful function calls.
