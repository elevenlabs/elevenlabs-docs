...
subtitle: Learn how to use webhooks to receive real-time notifications when your speech-to-text tasks are completed.
---
...
- Basic understanding of the [Speech-to-text API](/docs/api-reference/speech-to-text)
...
<Steps>
  ...
  <Frame background="subtle" caption="Create a new webhook in the ElevenLabs dashboard">
    <img src="/assets/images/cookbooks/scribe/webhooks/create-webhook.png" alt="Create webhook interface" />
  </Frame>
  ...
  <Frame background="subtle" caption="Associate webhook with speech-to-text tasks">
    <img src="/assets/images/cookbooks/scribe/webhooks/associate-webhook.png" alt="Webhook association interface" />
  </Frame>

  ### Make API calls with webhook enabled
...
app.post('/webhook/speech-to-text', (req, res) => {
  try {
    ...
    const { task_id, status, text, language_code } = req.body;
