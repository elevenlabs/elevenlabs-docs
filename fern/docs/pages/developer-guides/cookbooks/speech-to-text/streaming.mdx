...
The difference between this and the [synchronous](/docs/developer-guides/cookbooks/speech-to-text/synchronous) example is that the streaming example returns the result as a stream of data. As such the latency between the transcription and the result is lower as it's streamed back in chunks rather than waiting for the entire response.
