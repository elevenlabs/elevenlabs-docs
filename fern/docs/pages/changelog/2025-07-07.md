...
- **Post-call Audio**: Added support for returning call audio in [post-call webhooks](/docs/conversational-ai/workflows/post-call-webhook), enabling comprehensive conversation analysis and quality assurance workflows.
...
- **SIP Trunk Enhancements**: Added support for separate inbound and outbound [SIP trunk configurations](/docs/conversational-ai/guides/sip-trunking) with enhanced access control and transfer options.
...
- [SIP trunk configuration](/docs/conversational-ai/guides/sip-trunking) now uses separate inbound and outbound trunk configs instead of single configuration
...
- Audio data in [post-call webhooks](/docs/conversational-ai/workflows/post-call-webhook) (configurable)
- [MCP (Model Context Protocol)](/docs/conversational-ai/customization/mcp/guide) tool call details in conversation history
...
- Added confidence scoring with `loss` field for words and overall transcript in [Forced alignment](/docs/api-reference/forced-alignment/convert)

#### Usage Analytics

- Added reporting workspace ID breakdown for character stats in [Get characters usage metrics](/docs/api-reference/usage/get-characters-usage-metrics)

#### Tool Configuration

- [Client tool](/docs/conversational-ai/customization/tools/overview) response timeout increased from 30 to 120 seconds
