...
The reason for this is that the Multilingual v2 model has been trained to recognize and read out numbers in a way that is more natural for human listeners, whereas the Flash v2.5 model is a much smaller model which has not.
