...
When using Text to Speech with complex items like phone numbers, zip codes and emails they might be mispronounced. This is often due to the model seeing the item for the first time. This guide will clarify when those discrepancies happen and how to have them pronounced correctly.
...
The reason for this is that the Multilingual v2 model has been trained to recognize and read out numbers in a way that is more natural for human listeners, whereas the Flash v2.5 model is a much smaller model which has not. This is not a design flaw, but rather a conscious decision in order to make the Flash model faster and to consume less credits.
