...
When using Text to Speech in the ElevenLabs dashboard or when using Conversational AI, you may have noticed that the audio output doesn't always sound as expected. This is often due to the text not being normalized correctly. For example, phone numbers and currencies can be read out in a way that isn't clear for the user. This guide will clarify when those discrepancies happen and how to mitigate them.
...
The reason for this is that the Multilingual v2 model has been trained to recognize and read out numbers in a way that is more natural for human listeners, whereas the Flash v2.5 model has not. This is not a design flaw, but rather a conscious decision in order to make the Flash model faster and to consume less credits.
...
def normalize_text(text: str) -> str:
    ...
    def money_replacer(match):
        ...
        num_in_words = p.number_to_words(int(num))
        return f"{num_in_words} {currency_map.get(currency_symbol, 'currency')}"

    text = re.sub(r"([$£€¥])(\d+)", money_replacer, text)
...
# Example usage
print(normalize_text("$1000")) # "one thousand dollars"
...
function normalizeText(text: string): string {
    return text
        ...
        .replace(/([$£€¥])(\d+)/g, (_, currency, num) => {
            ...
            return `${toWords(n)} ${currencyMap[currency] || "currency"}`;
        })
...
function spellOutDigits(num: string): string {
    return num.split("").map(digit => toWords(Number.parseInt(digit))).join(" ");
}

// Example usage
console.log(normalizeText("$1000"));   // "one thousand dollars"
