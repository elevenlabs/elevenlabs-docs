...
<Warning title="Advanced">
  Orchestrating voice agents using this multi-context WebSocket API is a complex task
...
<Warning>
  The multi-context WebSocket API is optimized for voice applications and is not intended
...
<Note>
  These best practices are essential for building responsive, efficient voice agents with
