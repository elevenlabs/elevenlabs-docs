---
title: Conversational agents best practices
...
<Warning title="Advanced">
  Orchestrating conversational agents using this multi-context WebSocket API is a complex task
  recommended for advanced developers. For a more managed solution, consider exploring our
  [Conversational AI product](/docs/conversational-ai/overview), which simplifies many of these
  challenges.
</Warning>
...
<Warning>
  The multi-context WebSocket API is optimized for conversational applications and is not intended
  for generating multiple unrelated audio streams simultaneously. Each connection is limited to 5
...
async def send_text_in_context(websocket, text, context_id, voice_settings=None):
    ...
    print(f"Sent text to context '{context_id}': {text[:30]}...")
...
async def continue_context(websocket, text, context_id):
    ...
    print(f"Added to context '{context_id}': {text[:30]}...")
...
async def flush_context(websocket, context_id):
    ...
    print(f"Flushed context '{context_id}'")
...
async def conversation_agent_demo():
    ...
    async with websockets.connect(
        WEBSOCKET_URI,
        max_size=16 * 1024 * 1024, # Adjusted to be 16MB, aligned with typical WebSocket limits
...
function sendTextInContext(websocket, text, contextId, voiceSettings = null) {
  ...
  console.log(`Sent text to context '${contextId}': ${text.substring(0, 30)}...`);
}
...
function continueContext(websocket, text, contextId) {
  ...
  console.log(`Added to context '${contextId}': ${text.substring(0, 30)}...`);
}
...
function flushContext(websocket, contextId) {
  ...
  console.log(`Flushed context '${contextId}'`);
}
...
async function conversationAgentDemo() {
  ...
  websocket.on('message', (message) => {
    try {
      ...
      if (data.audio) {
        console.log(`Received audio for context '${contextId}'`);
      }
