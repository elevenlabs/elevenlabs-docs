...
When creating your dub, you have a special option during the creation process that is only available to the dubbing studio: manual dubbing. This option allows you to create a manual dub where you upload all of the files individually. You can upload the video file, the background audio, and the audio of only the speakers. Additionally, you need to include a CSV file indicating the names of the speakers, the start and end time of when they are speaking, the original text, and the translated text. It's similar to a subtitle file but with a lot more information. This file needs to adhere to a very strict format to work correctly.
