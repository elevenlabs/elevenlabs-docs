...
| **Verbatim** | Legal, research | Attempts to capture <i>exactly</i> what is said, including all filler words, stutters and audio event tags. |
  ...
  <Step title="Order transcript">
    <AccordionGroup>
      <Accordion title="Transcribing new files">
        ...
        You can also select the 'Human Transcript' option in the [Speech to Text](/docs/pages/capabilities/speech-to-text.mdx) order dialog.
        <Frame background="subtle">
  ...
  <Step title="Export transcript">
    ...
    <AccordionGroup>
      <Accordion title="Quick export">
          Open a transcript on your [Productions](https://elevenlabs.io/app/productions) page and click the three dots -> "Export."
          <Frame background="subtle">
...
All prices are USD ($) and per minute of source audio.
...
**Note:** prices are subject to change. You will always see the final price for an order during the checkout process.
...
<AccordionGroup>
  <Accordion title="Non-verbatim">
    ...
    **Note:** This is the default option for Productions transcriptions. Unless you explicitly select 'Verbatim' mode, we will deliver a non-verbatim transcript.
...
<AccordionGroup>
  ...
  <Accordion title="Can I make changes once I receive the final version?">
    For now, no. You can export a completed transcript and make changes off platform. We plan to add support for this soon.
