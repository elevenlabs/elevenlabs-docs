...
The difference between this and the [synchronous](/docs/cookbooks/speech-to-text/synchronous) example is that the streaming example returns the result as a stream of data. As such the latency between the transcription and the result is lower as it's streamed back in chunks rather than waiting for the entire response.
...
transcription = client.speech_to_text.convert_as_stream(
    model_id="scribe_v1", # "scribe_v1_experimental" is also available
    file=audio_data
)
...
const transcription = await client.speechToText.convertAsStream({
  file: audioBlob,
  model_id: 'scribe_v1', // Model to use, "scribe_v1_experimental" is also available
