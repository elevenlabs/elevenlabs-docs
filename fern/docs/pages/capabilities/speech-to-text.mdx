...
- `spacing` - The space between words, not applicable for languages that don't use spaces like Japanese, Chinese, Thai, Lao, Burmese and Cantonese
...
<AccordionGroup>
  ...
  <Accordion title="High Accuracy (>5% to ≤10% WER)">
    ...
    Gujarati (guj), Hungarian (hun), Kazakh (kaz), Latvian (lav), Lithuanian (lit), Marathi (mar),
    Nepali (nep), Odia (ori), Persian (fas), Slovenian (slv), Tamil (tam) and Telugu (tel)
  </Accordion>
  <Accordion title="Good (>10% to ≤25% WER)">
    Afrikaans (afr), Arabic (ara), Armenian (hye), Assamese (asm), Asturian (ast), Azerbaijani
    (aze), Burmese (mya), Croatian (hrv), Georgian (kat), Hausa (hau), Hebrew (heb),
  ...
  <Accordion title="Moderate (>25% to ≤50% WER)">
    Amharic (amh), Fulah (ful), Ganda (lug), Igbo (ibo), Irish (gle), Khmer (khm),
