...
<AccordionGroup>
  ...
  <Accordion title="What qualifies as a free regeneration?">
    ...
    - Any changes to the text, voice settings, or other parameters will require a new generation (that is not free)

    Free regenerations are useful in case there is a slight distortion in the audio output. According to ElevenLab's internal benchmarks, regenerations will solve roughly half of issues with quality, with remaining issues usually due to poor training data.
