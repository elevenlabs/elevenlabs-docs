...
Using the new [Eleven v3 model](/docs/models#eleven-v3-alpha), voices that are capable of a wide range of emotion can be designed via a prompt. When using the v3 model, you can optionally provide reference audio to provide context like a specific accent or speaking style.

Additionally, Eleven v3 allows you to "remix" existing voices to create new ones. For example, you can remix an existing voice to create a new one with a different accent, speaking style or pitch.

<Note>
  Voice design with v3 is currently in alpha. You can only remix voices that were created by you, for example previously designed voices, IVC and PVC voices.
</Note>
