...
To adjust voice settings from the [Speech Synthesis](https://elevenlabs.io/app/speech-synthesis) page on mobile devices, click the `Voice Settings` button located near the bottom-right corner of the screen. For desktop browsers, switch from `SIMPLE` to `ADVANCED` mode using the toggle located at the upper-right corner of the page. Hovering over either label above a voice settings slider (e.g., `More variable` or `More stable` for Stability) will display a tooltip summarizing the descriptions below.
