...
<Info>
  **2024-02-28:** Currently, we are experiencing longer wait times for
  Multilingual as we just released the new technology for all voices. Thus, we
  are expected to have a longer wait time until we get through the backlog of
  voices waiting to be trained. This should only be temporary, and training
  times will stabilize and go back to the above estimate in a few days' time.
</Info>
