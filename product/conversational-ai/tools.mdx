...
- Schedule appointments on someone's calendar
- Book restaurant reservations
- Create or update customer records in a Customer
- Get inventory data to make product recommendations
- Fetch the latest football score, stock price, weather data or any other realtime information you need

To help you get started with Tools, we'll walk through an "AI receptionist" we created by integrating with the Cal.com API. 

<iframe width="100%" height="400" src="https://www.youtube.com/embed/D02nBMkrG2U" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
...
<img height="200" src="/conversational-ai/images/bearer_secret.png" />
...
(1) We want our AI receptionist to be able to answer questions like "When is Sam available to meet on tomorrow?" or "Is Sam free at 10:30am on Tuesday?". To make this possible, we integrated the Cal.com [Get Available Slots](https://cal.com/docs/api-reference/v2/slots/get-available-slots#get-available-slots) endpoint.

Name: Get_Available_Slots

Description: This API takes a date / time range as input and returns open calendar slots within that range.

Method: Get

URL: https://api.cal.com/v2/slots/available

(2) Once our caller and receptionist have agreed on a suitable time to meet, we'd like to book a meeting. For this we use the [Create a booking](https://cal.com/docs/api-reference/v2/bookings/create-a-booking#create-a-booking) endpoint. 

Name: Book_Meeting

Description: Call this function once you have collected the caller's full name, meeting time, and email address.
...
<img height="200" src="/conversational-ai/images/headers.png" />
...
<img height="200" src="/conversational-ai/images/cal_get_slots.png" />
...
<img height="200" src="/conversational-ai/images/cal_book_meeting.png" />
...
|-------------|-----------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| startTime   | String    | Yes      | The start time of the slot the person is checking availability for in UTC timezone, formatted as ISO 8601 (e.g., '2024-08-13T09:00:00Z'). Extract time from natural language and convert to UTC. |
| eventTypeId | String    | Yes      | The desired meeting length, as an event id. If 15 minutes, return 1351800. If 30 minutes, return 1351801. If 60 minutes, return 1351802. If you are not sure, return 1351801.                    |
